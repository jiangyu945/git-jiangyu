...
int main(int argc,char **argv)
{
    ...
    //ev_io_init(&sock_w,sock_cb,listenfd,EV_READ);  
    ev_io_start(poller,&sock_w);//启动观察事件

    ev_init(&signal_w,signal_cb);
    ev_signal_set(&signal_w,SIGINT);

    ev_set_io_collect_interval(poller,0.1);//设置轮询的时间,默认为0，系统以最小时间调度轮询，此值越大延迟越大，但能提高效率。通常设为0.1即能满足时效和提高效率（当然游戏除外）
    ev_run(poller,0);
