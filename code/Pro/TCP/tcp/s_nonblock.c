 ...
 Version     : v2.2
 date        : 2018-10-31
 Description : Multipthread Socket Server (NONBLOCK)
 ============================================================================
 */
#include <stdio.h>
#include <stdlib.h>
...
int get_one_frame(int s_fd)
{
    ...
    {
        //接收数据  
        nByte = recv( s_fd, recv_tmp, sizeof(recv_buf), MSG_DONTWAIT );
        //printf("nByte:%d\n",nByte);
        if(nByte>0)           
        {
            for(i=0;i<nByte;i++)
...
void* pthread_handler(void* sock)
{
    ...
    { 
        ...
        //发送数据请求    
        ret = send(newsock,req_buf,REQ_FRAME_SIZE,MSG_DONTWAIT);  //flags设置为MSG_DONTWAIT，以非阻塞方式发送
        if(ret == 0){  //客户端关闭连接
            close(newsock);   
            return NULL;   
        }
